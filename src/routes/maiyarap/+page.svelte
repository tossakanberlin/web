<div class="flex flex-col justify-between items-center bg-blue text-white py-10">
	<div class="grid grid-cols-1 md:grid-cols-2 gap-10">
		<div class="visible md:invisible md:absolute m-10">
			<img
				src="maiyarap_logo.png"
				class="md:absolute visible md:invisible max-h-full w-full object-cover top-0 left-0"
				alt="maiyarap"
			/>
		</div>
		<div
			class="flex flex-col justify-between text-center md:text-start md:items-start mr-10 ml-10 font-serif"
		>
			<div class="grid text-xl gap-10">
				<div
					class="grid grid-cols-1 md:grid-cols-2 justify-between md:justify-start text-center md:text-start font-serif md:gap-0 gap-10"
				>
					<div class="flex flex-col">
						<h2 class="text-6xl">MAIYARAP X-BERG</h2>
						<h2 class="text-4xl">ORANIENSTRASSE 200, 10999 BERLIN</h2>
					</div>
				</div>
				<div>
					<h2 class="text-5xl">THAI NOODLE BAR</h2>
					<div class="flex flex-col md:flex-row gap-5">
						<h2 class="font-sans text-5xl">thai noodle soup aka</h2>
						<h2 class="font-sans text-5xl bg-orange">gieuw tieuw</h2>
					</div>
					
				</div>
				
				<div class="grid grid-cols-1 md:grid-cols-2 text-3xl">
					<h2>OPEN DAILY</h2>
					<h2>12:00 - 22:00</h2>
					<h2>KITCHEN CLOSE</h2>
					<h2>21:00</h2>
				</div>
				<div class="flex md:flex-row flex-col">
					<h2 class="text-3xl font-sans">*no reservations, just walk ins 😊</h2>
					<button
							type="submit"
							class=" bg-pink rounded-md shadow-none font-serif text-2xl mx-5 sm:mt-0 place-self-center my-5 py-1 px-5 w-[15rem]"
							>MENU
						</button>
				</div>
				
			</div>
		</div>
		<div class="invisible md:visible md:relative m-10">
			<img
				src="maiyarap_logo.png"
				class="md:absolute max-h-full w-full object-contain top-0 left-0"
				alt="maiyarap"
			/>
		</div>
	</div>
</div>

<div class="flex flex-col justify-between items-center bg-white text-blue py-10 gap-10 mx-10">
	<div class="flex flex-col justify-between text-center font-sans text-3xl gap-5">
		<p>If you´ve been to Thailand, you know how much we love our noodle soups.</p>
		<p>You´ll see Gieuw Tieuw sold in every corner 24/7 in Bangkok. That´s how great, diverse & hearty our noodle soups are.</p>
		<p>Here at Maiyarap, our new location in Kreuzberg, we offer only the best of the best.</p>
		<p>Gieuw Tieuw paired with our signature dumplings and unique cocktails🥂</p>
	</div>
	<div class="flex max-h-[540px] w-full mb-5">
		<img
				src="maiyarap_prev_1.jpg"
				class=" max-h-full w-full object-cover"
				alt="maiyarap_noodle"
			/>
	</div>
	
</div>

<style lang="postcss">
	:global(html) {
		background-color: theme(colors.white);
	}
</style>
